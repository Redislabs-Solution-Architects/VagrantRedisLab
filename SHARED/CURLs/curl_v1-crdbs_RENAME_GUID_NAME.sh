...
test "$1" = '' && echo "Execution is: $0 <GUID> <NAME>"
test "$1" = '' && exit 1
test "$1" = '' && echo "Execution is: $0 <GUID> <NAME>"
test "$2" = '' && exit 1
...
CURL="curl -o $0.json -s -k -u $REDIS_cluster_admin:$REDIS_cluster_password -X PATCH -H 'Content-Type: application/json' -d '{\"default_db_config\":{\"name\": \"$2\"}, \"name\": \"$2\" }' https://$REDIS_cluster_fqdn:9443/v1/crdbs/$1"
