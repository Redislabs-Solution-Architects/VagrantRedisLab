...
while test -f /root/RES_install.sh_running; do
    echo "$(date) - Exists /root/RES_install.sh_running. sleep 20."
    echo
    tail -1 /root/RES_install.sh_log
    echo
    sleep 20
done
...
hostname -I | grep $REDIS_first_node_ip
hostname_result=$?
...
test $hostname_result -eq 0 && (
    rladmin cluster create name $REDIS_cluster_fqdn username $REDIS_cluster_admin password $REDIS_cluster_password addr $REDIS_self_IP external_addr $REDIS_self_IP
    sleep 5
) ||
